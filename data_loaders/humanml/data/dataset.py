...
from data_loaders.humanml.scripts.motion_process import recover_from_ric, recover_rot_quat
...
class Text2MotionDatasetV2(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer):
        ...
        all_traj_tensor = torch.cat(all_traj, dim=0).reshape((-1, 3))
    ...
    def __getitem__(self, item):
        ...
        if m_length < self.max_motion_length:
            ...
            traj_data = torch.cat((traj_data, torch.zeros(self.max_motion_length - m_length, 6, 3)), dim=0)
...
class HumanML3D(data.Dataset):
    def __init__(self, mode, datapath='./dataset/humanml_opt.txt', split="train", args = None, **kwargs):
        self.mode = mode
        ...
        elif mode in ['train', 'eval', 'text_only', 'train_code']:
            # used by our models
        ...
        elif mode == 'train_code':
            self.w_vectorizer = WordVectorizer(pjoin(abs_base_path, 'glove'), 'our_vab')
            self.t2m_dataset = Text2MotionDatasetV2(self.opt, self.mean, self.std, self.split_file, self.w_vectorizer, args)
