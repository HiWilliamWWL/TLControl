...
from data_loaders.humanml.motion_loaders.comp_v6_model_dataset import CompMDMGeneratedDataset_MTM_Only
from data_loaders.humanml.motion_loaders.comp_v6_model_dataset import CompMDMGeneratedDataset_MTM_Only_UpdateMax


from data_loaders.humanml.utils.word_vectorizer import WordVectorizer
import numpy as np
from data_loaders.humanml.motion_loaders.comp_v6_model_dataset import CompV6GeneratedDataset
...
# our loader
def get_mtm_loader(transformer_traj_model, net, batch_size, ground_truth_loader, mm_num_samples, mm_num_repeats, max_motion_length, num_samples_limit, using_Root_Only = False):
    opt = {
        'name': 'test',  # FIXME
    }
    print('Generating %s ...' % opt['name'])
    print(f"Num of repeats is {mm_num_repeats}")
    
    dataset = CompMDMGeneratedDataset_MTM_Only(transformer_traj_model, net, ground_truth_loader, mm_num_samples, mm_num_repeats, max_motion_length, num_samples_limit, using_Root_Only = using_Root_Only)
    ...
    mm_motion_loader = DataLoader(mm_dataset, batch_size=1, num_workers=1, shuffle=False)
