...
def build_evaluators(opt):
    ...
    checkpoint = torch.load("./t2m/text_mot_match/model/finest.tar",
                            map_location=opt['device'])
    movement_enc.load_state_dict(checkpoint['movement_encoder'])
    text_enc.load_state_dict(checkpoint['text_encoder'])
    motion_enc.load_state_dict(checkpoint['motion_encoder'])
...
class EvaluatorMDMWrapper(object):
    ...
    def get_motion_embeddings2(self, motions, m_lens):
        #with torch.no_grad():
        ...
        m_lens = m_lens // self.opt['unit_length']
        motion_embedding = self.motion_encoder(movements, m_lens)
        return motion_embedding, align_idx
    
    def get_motion_embeddings3(self, motions, m_lens, align_idx):
        #with torch.no_grad():
        motions = motions.to(self.device).float()
